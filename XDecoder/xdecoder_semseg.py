...
def load_xdecoder_model(device):
    pretrained_pth = 'weights/xdecoder_focalt_best_openseg.pt'
    opt = load_opt_from_config_files(["configs/xdecoder/svlp_focalt_lang.yaml"])
...
def main(args=None):
    ...
    classes = ['rocking chair', 'teddy bear', 'grass', 'white wall', 'other']
    ...
    with torch.no_grad():
        ...
        plot = plt.imshow(sem_seg)
        values = np.unique(sem_seg.ravel())
